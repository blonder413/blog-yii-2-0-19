...
class TypeController extends Controller
{
  ...
  //                    ],
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ( $model->save() ) {
                Yii::$app->session->setFlash('success', 'Type created successfully');
            } else {
                Yii::$app->session->setFlash('danger', "Type can't be created");
            }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                Yii::$app->session->setFlash('success', Yii::t('app', 'Type udpated successfully'));
            } else {
                $errors = '';
                  foreach ($model->getErrors() as $key => $value) {
                      foreach ($value as $row => $field) {
