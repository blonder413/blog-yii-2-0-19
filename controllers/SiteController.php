...
//            }
            ...
            $oldComment = Comment::find()->where(
                ...
                [ ":email" => Security::encrypt($comment->email), ":article" => $comment->article_id, ":comment" => $comment->comment ]
            )->count();
...
//            Yii::$app->response->setDownloadHeaders($filename);
    ...
    /**
     * find all articles by the author
     * @param Integer $id the created_by attribute
     * @return array with the articles of the author
     */
    public function actionAuthor($id)
    {
        $user = User::find()->where('name = :id', [':id' => urldecode($id)])->one();
        $query = Article::find()->where( 'created_by = :id', [':id' => $user->id] );
        $pagination     = new Pagination([
            'defaultPageSize'   => 15,
            'totalCount'        => $query->count(),
        ]);
